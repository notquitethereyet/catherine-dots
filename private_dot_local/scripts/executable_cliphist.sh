...
# Helper function to display fuzzel
fuzzel_menu() {
    fuzzel --dmenu
}
...
if [ "$1" = "c" ]; then
    selected_item=$(cliphist list | fuzzel_menu)
...
case "${main_action}" in
    ...
    "View Favorites")
        ...
        ;;
    "Manage Favorites")
        manage_action=$(echo -e "Add to Favorites\nDelete from Favorites\nClear All Favorites" | fuzzel_menu)
        case "${manage_action}" in
            "Add to Favorites")
                item=$(cliphist list | fuzzel_menu)
                if [ -n "$item" ]; then
