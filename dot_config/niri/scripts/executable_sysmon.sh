...
# Fetch the preferred terminals from niri keybindings configuration
term=$(grep -m 1 '^\$term =' "$HOME/.config/niri/hypr/keybindings.conf" | sed 's/.*\$term = //' | xargs)
term2=$(grep -m 1 '^\$term2 =' "$HOME/.config/niri/hypr/keybindings.conf" | sed 's/.*\$term2 = //' | xargs)
